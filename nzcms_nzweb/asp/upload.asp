...
Sub DoSave()
	...
	If makeImageSLT(sUploadDir, sSaveFileName, s_SmallImageFile) = True Then
		...
		s_SmallImageScript = "try{obj.addnzcms_up('" & sOriginalFileName & "', '" & s_SmallImageFile & "', '" & s_SmallImagePathFile & "');} catch(e){} "
	Else
		s_SmallImageFile = ""
		Call makeImageSY(sUploadDir, sSaveFileName)
	End If
	...
	sOriginalFileName = Replace(sOriginalFileName, """", "\""")
	Call OutScript("parent.UploadSaved('" & sPathFileName & "','" & s_SmallImagePathFile & "');var obj=parent.dialogArguments;if((!obj.eWebEditor)||(!obj.eWebEditor_Temp_HTML)||(!obj.eWebEditor_UploadForm)){obj=parent.dialogArguments.dialogArguments;} try{obj.addnzcms_up('" & sOriginalFileName & "', '" & sSaveFileName & "', '" & sPathFileName & "');} catch(e){} " & s_SmallImageScript)
End Sub
...
Sub DoRemote()
	...
	End If
	Response.Write "<html><head><meta http-equiv='Content-Type' content='text/html; charset=gb2312'></head><body>" & _
	"<input type=hidden id=UploadText value=""" & inHTML(sContent) & """>" & _
	"</body></html>"
	Call OutScriptNoBack("parent.setHTML(UploadText.value);try{parent.addnzcms_up('" & sOriginalFileName & "', '" & sSaveFileName & "', '" & sPathFileName & "');} catch(e){} parent.remoteUploadOK();")
End Sub
...
Sub DoUpload_LyfUpload()
	...
	Set oUpload = Server.CreateObject("LyfUpload.nzcms_up")
	...
	sResult = oUpload.SaveFile("nzcms_up", sUploadDir, True, sSaveFileName)
	Select Case sResult
	Case "0"
...
Sub DoUpload_SAFileUp()
	...
	If oFileUp.Form("nzcms_up").TotalBytes > nAllowSize*1024 Then
		Err.Clear
		Call OutScript("parent.UploadError('size')")
	End If
	If oFileUp.Form("nzcms_up").IsEmpty Then
		Call OutScript("parent.UploadError('file')")
	End If
	Dim sShortFileName
	sShortFileName = Mid(oFileUp.Form("nzcms_up").UserFilename, InstrRev(oFileUp.Form("nzcms_up").UserFilename, "\") + 1)
	sFileExt = LCase(Mid(sShortFileName, InStrRev(sShortFileName, ".") + 1))
	Call CheckValidExt(sFileExt)
	sOriginalFileName = sShortFileName
	sSaveFileName = GetRndFileName(sFileExt)
	oFileUp.Form("nzcms_up").SaveAs (sUploadDir & sSaveFileName)
	Set oFileUp = Nothing
End Sub
...
Sub DoUpload_ASPUpload()
	...
	If Err.Number = 8 Then
		...
		Call OutScript("parent.UploadError('size')")
	End If
	Set oFile = oUpload.Files("nzcms_up")
...
Sub DoUpload_Class()
	...
	If oUpload.Err > 0 Then
		...
		End Select
	End If
	Set oFile = oUpload.File("nzcms_up")
